...
<%@ page import="server.domain.NumericType" %>
...
<html:form action="/pages/manageProfilesEdit-submit" onsubmit="selectAllOptionElements()"> 
    ...
    <html:errors />
    <fieldset>
        <legend style="color:black"><bean:message key="manageProfiles.profileoverview" /></legend>
...
if(edit){ 
%>
            <html:text property="name" size="60" readonly="true" errorStyleClass="error" />
<% 
}else{
%>
...
%>
        ...
        <div>
            ...
            <html:text property="lastChange" size="60" readonly="true"/>		
        </div>
    ...
    <fieldset>
        <legend style="color:black">
            <bean:message key="manageProfiles.profileSettings" />
        </legend>
<%
...
if(edit || request.getSession().getAttribute("PROFILEERROR") == Boolean.TRUE)
{
...
}else
{
    try 
    {
		...
		if (request.getSession().getAttribute("currentUser") == null) {
	        ...
	        selectedProfile = controllerConnection.createDefaultProfileForUser(((User)request.getSession().getAttribute("currentUser")).getId());			
		}
	} 
    catch (Exception ex) 
    {
...
%>
        <h3><bean:message key="<%=componentName%>" /></h3>
    ...
    Collections.sort(componentAttributeValues, new Comparator() {
        public int compare(Object o1, Object o2)
        {
            ...
            if(c1 == Boolean.class)
                ...
                name1 = "b" + name1;
            else if(c1 == Integer.class && !a1.isSelectable())
                name1 = "c" + name1;
            ...
            if(c2 == Boolean.class)
                ...
                name2 = "b" + name2;
            else if(c2 == Integer.class && !a2.isSelectable())
                name2 = "c" + name2;
...
%>
        ...
        <div>
            <label><bean:message key="<%=attributeName %>" /></label>
	    
<%
        if (componentAttributeValue instanceof NumericValue) 
        {
		...
		if(componentAttribute.isSelectable())
		{
%>		
							    
            	<select name="<%=fieldName%>" id="<%=fieldName%>" border="5">
<%
			...
			for(int l=min; l<=max; l += step)
			{
				...
				{
%>
                	<option value="<%=l%>" selected><%=controllerConnection.getNetworkFactory().call(componentName, attributeName.toLowerCase(), new Object[]{new Integer(l)})%></option>
<%
				}else
				{
%>
                	<option value="<%=l%>"><%=controllerConnection.getNetworkFactory().call(componentName, attributeName.toLowerCase(), new Object[]{new Integer(l)})%></option>
<%
				}
			}
%>
		...
		<div><bean:message key="manageProfiles.selected"/></div>
<%
		}else
		{
%>
       		<input type="text" 
				   id="<%=componentAttribute.getId()%>" 
				   name="<%=fieldName%>" 
				   value="<%=defaultValue %>" />

        	<input type="button" 
				   value="+"
				   onclick="numericChange(<%=componentAttribute.getId()%>,
								   <%=numericType.getMin()%>,
								   <%=numericType.getMax()%>,
								   <%=numericType.getStep()%>,
									'+')" />
...
<%
	}else if (componentAttributeValue instanceof StringValue) {
            Object defaultValue = componentAttributeValue.getValue();
%>
       		<input type="text" 
				   id="<%=componentAttribute.getId()%>" 
...
<%			
        }else if (componentAttributeValue instanceof BooleanValue) 
        {
            ...
            if (defaultValue.booleanValue()) 
               checked = "checked";
%>
      	    <input type="checkbox" name="<%=fieldName%>" <%=checked %> value="on"/>
        </div> 
<%
      	}else if (componentAttributeValue instanceof ListValue) 
        {
            ...
            boolean isVideo     = listType.isVideoList().booleanValue();
            if(isVideo)
                videoLists += fieldName + ";";
            else
                noVideoLists += fieldName + ";";
%>
        </div>
        <div id="list">
            <div class="addmember">
            <label><bean:message key="manageProfiles.selectedFiles" /></label>
            <select name="<%=fieldName%>" id="<%=fieldName%>" size="10"  border="5" multiple>
<%
            ...
            for(int k=0; k<list.size(); k++) 
            {
                ...
                if (value != null && value.length()>0)
                {
                    if(value.indexOf("<file>") > 0)
                        value = value.substring(value.lastIndexOf("<file>" + 6));
                    if(value.indexOf("</file>") > 0)
...
%>
        ...
        <div class="arrow">
            <input type="button" name="addToList" value="  --&gt;  " onclick="javascript:AddToList(<%=isVideo%>, '_<%=fieldName%>_','<%=fieldName%>','<bean:message key='errors.invalidListCombination' />')">
            <input type="button" name="removeFromList" value="  &lt;--  " onclick="javascript:RemoveFromList('<%=fieldName%>')">
        </div>

        <!-- Create list with all mediacontent, to which user has access -->	
        <div class="addmember">
            <label><bean:message key="manageProfiles.availableFiles" /></label> 
            <select name="_<%=fieldName%>_" id="_<%=fieldName%>_" size="10" border="5" multiple>
<%
...
%>
        ...
        </div></div> <!-- das ist hoffentlich das div das "list" schließt -->
<%	
 		}
    }
}
if(videoLists.length() > 0) videoLists = videoLists.substring(0, videoLists.length()-1);
if(noVideoLists.length() > 0) noVideoLists = noVideoLists.substring(0, noVideoLists.length()-1);
%>  
    </fieldset>

    <script language="javascript">
    setVideoLists("<%=videoLists%>");
    setNoVideoLists("<%=noVideoLists%>");
    </script>
